        ...
        <>
            ...
            <div className="md:hidden space-y-4">
                {vouchers.length === 0 ? (
                    ...
                    vouchers.map((voucher) => (
                        <div key={voucher.id} className="bg-card rounded-lg border p-4 space-y-3">
                            <div className="flex items-start gap-3">
                                ...
                                <div className="flex-1 min-w-0">
                                    <div className="flex items-start justify-between">
                                        <div>
                                            <h3 className="font-medium truncate pr-2">{voucher.name}</h3>
                                            <p className="text-sm text-muted-foreground font-mono">{voucher.code}</p>
                                        </div>
                                        <Badge variant={voucher.status === 'active' ? 'success' : 'secondary'} className="shrink-0">
                                            {voucher.status === 'active' ? 'Hoạt động' : 'Ngưng'}
                                        </Badge>
                                    </div>
                                    <div className="flex items-center gap-2 mt-1">
                                        ...
                                        <Badge variant={voucher.type === 'percentage' ? 'info' : 'secondary'} className="text-xs">
                                            {voucher.type === 'percentage' ? 'Phần trăm' : 'Cố định'}
                                        </Badge>
                                    </div>
                                </div>
                            ...
                            <div className="grid grid-cols-2 gap-2 text-sm pt-2 border-t">
                                <div className="flex items-center justify-between">
                                    ...
                                    <span>{voucher.used_count || 0}/{voucher.quantity}</span>
                                </div>
                                <div className="flex items-center justify-between">
                                    <span className="text-muted-foreground">Hết hạn:</span>
                                    <span>{voucher.end_date}</span>
                                </div>
                            </div>

                            <div className="flex justify-end gap-2 pt-2 border-t">
                                <Button variant="outline" size="sm" onClick={() => onEdit(voucher)} className="flex-1">
                                    <Edit className="h-4 w-4 mr-2" />
                                    Sửa
                                </Button>
                                <Button variant="outline" size="sm" onClick={() => onDelete(voucher.id)} className="flex-1 text-red-500 hover:text-red-600 hover:bg-red-50">
